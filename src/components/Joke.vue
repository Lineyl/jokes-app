      ...
      >
        <figure
          ...
          class="
            ...
            bg-gray-50 text-gray-800 border-2-solid border-gray-200
            drop-shadow-md text-base opacity-50
            hover:opacity-100 transition-all duration-300
          "
        >
          <figcaption class="font-semibold tracking-wide">
...
import type { GetJokeResponse } from '@/libs/jokes/types';

import { JokeType } from '@/declarations';
import { jokesHttpClient } from '@/libs/jokes';
...
const getLoadingMessage = () => {
  const massegeIndex = Math.floor(Math.random() * LOADING_MESSAGES.length);
  return LOADING_MESSAGES[massegeIndex];
};
