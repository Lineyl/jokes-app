import type { GetCategoriesResponse, GetJokeResponse } from '@/libs/jokes/types';
...
class JokesHttpClient {
  ...
  public async getJoke(categories: string[]) {
    ...
    const response = await this.request<GetJokeResponse>('GET', '/joke');
  ...
  private async request<T = any>(
    ...
    try {
      ...
      console.error('Failed to send request to JokesAPI', error);
      return null;
    }
  }
